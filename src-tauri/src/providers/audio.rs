...
//!
//! Uses a 4096-sample FFT window with 1024-sample hop (STFT) for 4× better
...
/// Emits `audio-bands` Tauri events and writes to temp file.
pub fn start_audio_capture(app: AppHandle, bands: SharedBands) {
    thread::spawn(move || {
        run_capture_loop(app, bands);
    });
}

fn run_capture_loop(app: AppHandle, bands: SharedBands) {
    ...
    loop {
        ...
        let _ = app.emit("audio-bands", &smoothed);
